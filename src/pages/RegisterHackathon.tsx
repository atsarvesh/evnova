...
const RegisterHackathon = () => {
  ...
  const addMember = () => {
    if (memberInput.trim() && members.length < hackathon.maxTeamSize - 1) {
  ...
  const handleSubmit = () => {
    ...
    setSubmitted(true);
    toast({ title: "Registration successful!", description: `Team "${teamName}" registered for ${hackathon.title}` });
  };
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Max team size: {hackathon.maxTeamSize} members</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              ...
              <Button onClick={handleSubmit} className="w-full bg-gradient-to-r from-primary to-secondary text-primary-foreground">
                Register Team
              </Button>
            </CardContent>
          </Card>
